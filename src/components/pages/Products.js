...
function Products() {
    ...
    const handleAddToCart = (event, id) => {
        ...
        let currentProductObj = shoesListDetails.filter(x => x.id === id)[0];
        currentProductObj.quantity = 1;
        currentProductObj.selectedSize = currentProductObj.sizes[0];
        addToCart(currentProductObj);
        setAddToCartStatus(true);
    }

    const handleAddedNoAction = (event) => {
        event.preventDefault();
    }
